-- visuals.lua (clean version)
local Visuals = {}

function Visuals.Init(services, state, config, utils)

    local Players = services.Players
    local RunService = services.RunService
    local Camera = workspace.CurrentCamera

    state.screenButtons = state.screenButtons or {}
    ...
    --------------------------------------------------
-- TRACER (Beam tới player)
--------------------------------------------------

function Visuals.addTracer(targetObj, color)

    if not state.running or not targetObj then return end

    task.spawn(function()

        task.wait(0.4)

        if state.itemTracers[targetObj] then return end
    ...
    local function isTeleportable(name)
        return string.find(name, "PasswordPaper")
            or string.find(name, "KeyCard")
    ...
    local function createTeleportButton(target)
        ...
        conn = RunService.RenderStepped:Connect(function()
            ...
            if not state.running or not target or not target.Parent then
                screenGui:Destroy()
                conn:Disconnect()
                return
            end
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            local ok = utils.teleportToTarget(target)
    ...
    function Visuals.addVisuals(obj, kind, nameOverride)
        ...
        state.visualObjects[obj] = visuals
        ...
        if obj:IsA("Model") then
            ...
            utils.storeObjectConnection(obj, conn)
        end
    end
    ...
    function Visuals.removeVisual(obj)
        ...
        local v = state.visualObjects[obj]
        ...
        state.visualObjects[obj] = nil
        utils.clearObjectConnections(obj)
        
        -- remove tracer
        if state.itemTracers[obj] then
            local t = state.itemTracers[obj]

            pcall(function() t.Beam:Destroy() end)
            pcall(function() t.Att0:Destroy() end)
            pcall(function() t.Att1:Destroy() end)

            state.itemTracers[obj] = nil
        end
